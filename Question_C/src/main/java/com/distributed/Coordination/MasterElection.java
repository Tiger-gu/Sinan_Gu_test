...
public class MasterElection implements Watcher{
    ...
    // Each node will submit its candidacy for maser to ZooKeeper
    // by creating a ephemeral znode under the /election persistent
    // znode. The node assosiated with the first znode in the list under the /election znode
    // will be recognized as the master. Other nodes will automatically take over slave roles. 
    // A slave node can be promoted to the master role if the master node is down.
    protected void reelectMaster() throws KeeperException, InterruptedException{
        ...
        while(predecessorStat == null) {
            children = zookeeper.getChildren(ELECTION_ZNODE, false);
            ...
            if (smallestChild.equals(currentZnodeName)) {
                ...
                // Every slave node will keep a watchful eye on its preceding
                // ephemeral znode in the list under /election . Once that node is down/broken,
                // the slave node will immediately call for an re-election through
                // the zookeeper watcher.Therefore, when any node especially the master node is down,
                // a reelection will be held and one of the slave nodes will be promoted to the master role.
                // In this way, there will always be a master node regardless of network crashes.
                System.out.println("I am the slave");
                predecessorIndex = Collections.binarySearch(children, currentZnodeName) - 1;
                predecessorZnodeName = children.get(predecessorIndex);
                // This call registers a watcher on the preceding znode.
